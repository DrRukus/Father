...
def permutations(inputList):
    if len(inputList) == 2:
        ...
        for i in range(0, listLength):
            ...
            #print tempList[0]
            k = 0
            #print "J Iterations = {0}".format(factorial(len(inputList) - 1))
            for j in range(factorial(len(inputList) - 1)):
                tempList.append([inputList[i]])
                #print "Initial Temp Temp List"
                #print tempList
                #print "J = {0}; K = {1}".format(j, k)
                tempList[-1] = tempList[-1] + smallPerms[k]
                if k == range(len(tempList)):
                    k = 0
                else:
                    k += 1
            ...
            for j in range(len(tempList)):
                perms.append(tempList[j])
        #print "Perms Returned"
        #print perms
        return perms
